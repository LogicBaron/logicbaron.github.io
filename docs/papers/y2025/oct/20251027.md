...
AGI 라 부르기 위해서 인공지능이 갖추어야할 능력을 정의하고 평가 방식을 만들고 이를 통해 GPT-4, GPT-5 를 평가함.

GPT-4 는 20% 수준의 아주 낮은 점수인데 GPT-5 는 57% 나 된다!
...
보통 추론 능력을 획득해서 높은 정확도와 확신을 가지게 하는 것이 RL 을 이용한 post-train 의 목적인데, posttrain 에서 획득되는 이 능력은 사실 base model 이 이미 가지고 있다는 주장. posttrain 은 이 confidence 를 더욱 강화시켜 주는 역할이라 한다.

confidence 를 강화시켜주는 동작은 confidence sharpening 인데 대표적인 confidence sharpening 방법은 tmeperature를 낮추는 거임.

논문에서는 $p^\alpha$ 방식이 temperature 조절보다 낮다고 함. 왜냐하면, temperature 조절 방식은 이후 이어지는 조합의 다양성이 고려되지 않음.

예를 들어서 

- A 토큰을 생성하고 나면 following tokens 의 가능성이 0.01 인 것이 50가지 = 0.5
- B 토큰은 following tokens 가 가능성이 0.2 인것 2개 = 0.4 일 떄

low temperature 방식은 entropy 가 너무 높아서 오히려 별로인 A 를 선택한다는 것. 반면 $p^\alpha$ 방식은 B 를 선택한다.

$p^\alpha$ 에서 샘플링하는 방식은 잘 알려진 알고리즘 사용함. (intractable 분포임.)

꽤 성능이 좋아졌다고하는데 이런 작업을 할 때마다 이게 의미가 진짜 있는가? 하는 의문.
